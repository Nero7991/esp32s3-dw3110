...
#define DW3000_IRQ_TASK_STACK_SIZE 4096
#define DW3000_IRQ_TASK_PRIO	   (configMAX_PRIORITIES - 1)

static const char* LOG_TAG = "DW3000";
static bool dw3000_interrupt_enabled;
static SemaphoreHandle_t irq_semaphore;
static TaskHandle_t irq_task_hdl;
...
static void IRAM_ATTR dw3000_isr(void* args)
{
	BaseType_t xHigherPriorityTaskWoken = pdFALSE;
	xSemaphoreGiveFromISR(irq_semaphore, &xHigherPriorityTaskWoken);
	portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
}

static void dw3000_irq_task(void* pvParameters)
{
	while (true) {
		xSemaphoreTake(irq_semaphore, portMAX_DELAY);
		dw3000_spi_acquire();
		do {
			dwt_isr();
...
int dw3000_hw_init_interrupt(void)
{
	...
	if (irq_semaphore == NULL) {
		LOG_ERR("Could not create IRQ semaphore");
		return ESP_FAIL;
	}
...
void dw3000_hw_fini(void)
{
	...
	if (irq_task_hdl != NULL) {
		vTaskDelete(irq_task_hdl);
		irq_task_hdl = NULL;
	}
	if (irq_semaphore != NULL) {
		vSemaphoreDelete(irq_semaphore);
		irq_semaphore = NULL;
	}
#endif
